...
version: 2.0.0
description: Create a single draft post using a workspace voice profile and optional template. V2 - Agent-aware, routes via Agent→Job context or standalone execution.
category: creator
...
inputs_schema:
  ...
  properties:
    ...
    agent_id: { type: string, description: "V2 - Agent executing this skill (optional for backward compat)" }
...
## V2 Changes
- **Agent-aware execution:** If `agent_id` provided, this skill runs in the context of an Agent→Job workflow
- **State machine updated:** `Draft → Inbox → Approved → Scheduled → Published`
- **Safe Mode is workspace-level only** (not per-agent)
- **Role capabilities:** Viewer/Creator always route to Inbox; Editor/Admin can schedule if Safe Mode OFF
- **Agent memory:** Can use `bolta.remember` to store learnings about what works
...
- `final_state` (draft | inbox | approved | scheduled | published)
- `inbox_item_id` (if routed to inbox)
- `notes_for_reviewer` (optional context for human reviewer)
- `warnings` (array of any policy violations or constraints encountered)
